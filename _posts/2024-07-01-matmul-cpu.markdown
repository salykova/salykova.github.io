...
excerpt: "In this step by step tutorial we'll implement high-performance SGEMM (=fp32 matrix multiplication) in C from scratch and learn how to optimize and parallelize code on CPUs. The final multi-threaded SGEMM implementation outperforms OpenBLAS on a broad range of matrix sizes with peak performance of over 1 TFLOPS on Ryzen 7700."
date:   2024-07-01 11:35:01 +0200
author: Aman Salykov
usemathjax: true
---
**TL;DR**
The code from the tutorial is available at [sgemm.c](https://github.com/salykova/sgemm.c). This blog post is the result of my attempt to implement high-performance matrix multiplication on CPU while keeping the code simple and scalable. The implementation follows the [BLIS](https://en.wikipedia.org/wiki/BLIS_(software)) design, works for arbitrary matrix sizes, and outperforms [OpenBLAS](https://en.wikipedia.org/wiki/OpenBLAS) achieving over 1 TFLOPS of peak performance across a wide range of matrix sizes on AMD Ryzen 7700.
