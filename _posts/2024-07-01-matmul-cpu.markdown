...
---
**TL;DR**
The code from the tutorial is available at [GEMM.c](https://github.com/salykova/GEMM.c). This blog post is the result of my attempt to implement high-performance matrix multiplication on CPU while keeping the code simple and scalable. The implementation follows the [BLIS](https://en.wikipedia.org/wiki/BLIS_(software)) design, works for arbitrary matrix sizes, and outperforms [OpenBLAS](https://en.wikipedia.org/wiki/OpenBLAS) achieving over 1 TFLOPS across a wide range of matrix sizes on AMD Ryzen 7700.
